...
public class controlNaveTeclado : MonoBehaviour
{
	//speed stuff
	float speed;
	...
	public int maxSpeed;
	float accel, decel;
	public float misilSpeed=70;


	public int chocado=0;
	...
	void Start()
	{
		...
		GameObject.Find("Conjunto").GetComponent<texto>().depurar(
			"\n-INICIO DE JUEGO en xyz: "+this.transform.position.x+","+this.transform.position.y+","+this.transform.position.z);
	...
	void OnGUI() {
		if (chocado==1) {
			if (GUI.Button(new Rect(0, 150, Screen.width, 60), "Uppps!! has chocado. PULSA PARA INTENTAR DE NUEVO")) {
	...
	void FixedUpdate()
	{
		...
		if (Input.GetKey(KeyCode.Joystick1Button1) || Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)) {
		...
		Vector3 miV = -offsetDir * sqrOffset * 20 * Time.fixedDeltaTime;
		if ( (System.Single.IsNaN(miV.x)) || (System.Single.IsNaN(miV.y)) || (System.Single.IsNaN(miV.z)))  
		{
		} else {
			transform.GetChild(1).Translate(miV);
		
		}


		//(**************** this ***************) is what actually makes the whole ship move through the world!
		Vector3 miVGO = (offsetDir * sqrOffset * 50 + transform.GetChild(1).forward * speed) * Time.fixedDeltaTime;
		if ( (System.Single.IsNaN(miVGO.x)) || (System.Single.IsNaN(miVGO.y)) || (System.Single.IsNaN(miVGO.z)))  
		{
		} else {
	//		if (chocado==0) {
			transform.Translate(miVGO,Space.World);
